  ...
  val logic = during setup new Area {
    ...
    val harts = for (hartId <- 0 until HART_COUNT) yield new Area {
      ...
      val m = new Area {
        ...
        def getCsrFilter(indirectId: Int, targetCsr: Int, cond: Bool = True): CsrCondFilter = {
          assert(iregs.contains(targetCsr), s"${targetCsr} is not an indirect CSR alias")

          val filter = CsrCondFilter(targetCsr, (iselect === indirectId) && cond)
          filter.setName("INDIRECT_FILTER_" + targetCsr.toString + "_" + indirectId.toString, weak = true)
