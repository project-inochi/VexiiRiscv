...
class ParamSimple() {
  ...
  var withTranslatedBusTlb = false
...
//    lsuForkAt = 1
  ...
  def addOptions(parser: scopt.OptionParser[Unit]) = {
    ...
    opt[Unit]("with-translated-dbus-tlb") action { (v, c) => withTranslatedBusTlb = true }
  ...
  def pluginsArea(hartId : Int = 0) = new Area {
    ...
    if (withMmu) plugins += new TranslatedDBusAccessPlugin(withTlb = withTranslatedBusTlb)
    withMmu match {
