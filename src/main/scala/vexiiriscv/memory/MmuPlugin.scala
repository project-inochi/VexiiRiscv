    ...
    val ports = for(ps <- portSpecsSorted) yield new Composite(ps.rsp, "logic", false){
      ...
      val ctrl = new Area{
        ...
        when(requireMmuLockup) {
          val allow_mxr     = priv.implementHypervisor.mux(lineIsGuest.mux(vsstatus.mxr, False), False) || status.mxr
          val allow_sum     = priv.implementHypervisor.mux(lineIsGuest.mux(vsstatus.sum, status.sum), status.sum)
...
//      }
    refillPorts.map(_.rsp).foreach{o =>
      ...
      val fetch = for((level, levelId) <- spec.levels.zipWithIndex) yield new Area{
        ...
        val accessFault = !shadowReadError && (pteReadError || !pteFault && leafAccessFault)
        val guestFault = shadowReadError
